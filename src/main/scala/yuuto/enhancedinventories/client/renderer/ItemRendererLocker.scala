...
object ItemRendererLocker{
  final val instance:ItemRendererLocker = new ItemRendererLocker();
}
class ItemRendererLocker extends IItemRenderer{
  ...
  override def handleRenderType(item:ItemStack, t:ItemRenderType):Boolean={
    if(item.getItem() != Item.getItemFromBlock(ProxyCommon.blockLocker) &&
        item.getItem() != Item.getItemFromBlock(ModuleRefinedRelocation.sortingLocker))
      return false;
  ...
  def renderItem(item:ItemStack, x:Float, y:Float, z:Float){
    ...
    if(item.hasTagCompound()){
      info = new DecorationInfo(item.getTagCompound(), false);
    }else
      info = new DecorationInfo(false);
    //set gl11
    GL11.glPushMatrix();
