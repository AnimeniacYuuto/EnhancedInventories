...
@ChestContainer
class ContainerWorktable(val tile:TileWorktable, player:EntityPlayer) extends Container{
  val craftingMatrix:InventoryCrafting=tile.getCraftingMatrix();
  val craftResult:IInventory=tile.getCraftResult();
  val chestInventory:IInventory=tile.getInternalInventory();
  ...
  def init(){
    this.addSlotToContainer(new SlotSchematic(chestInventory, 27, 8, 35));
    this.addSlotToContainer(new SlotCraftingExtended(tile, this.playerInv.player, this.craftingMatrix, this.craftResult, 0, 124, 35));
    
    //int x = 0, y = 0;
    //Bind crafting matrix
    craftMatrixStart = this.inventorySlots.size();
    for (y <-0 until 3){
        for (x <-0 until 3){
            this.addSlotToContainer(new Slot(this.craftingMatrix, x + y * 3, 30 + x * 18, 17 + y * 18));
        }
    }
    //Bind chest inventory
    chestStart = this.inventorySlots.size();
    for (y <-0 until 3){
        for (x <-0 until 9){
            this.addSlotToContainer(new Slot(this.chestInventory, x + y * 9, 8 + x * 18, 84 + y * 18));
        }
    }
    ...
    for (y <-0 until 3){
        for (x <-0 until 9){
            this.addSlotToContainer(new Slot(this.playerInv, x + y * 9 + 9, 8 + x * 18, 142 + y * 18));
        }
    }
    for (x <-0 until 9){
        this.addSlotToContainer(new Slot(this.playerInv, x, 8 + x * 18, 200));
    }
