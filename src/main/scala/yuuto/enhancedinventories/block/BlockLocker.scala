 ...
 * @author Jacob
 */
class BlockLocker(name:String) extends BlockBaseEI(Material.wood, name)
  ...
  override def createNewTileEntity(world:World, meta:Int):TileEntity=new TileLocker(meta);
  ...
  override def onBlockActivated(world:World, x:Int, y:Int, z:Int, player:EntityPlayer, side:Int, hitX:Float, hitY:Float, hitZ:Float):Boolean={
    ...
    if(!tileEntity.isInstanceOf[TileLocker])
      return false;
    if(tileEntity.asInstanceOf[TileLocker].canPlayerAccess(player)){
      ...
      if(EIConfiguration.moduleRefinedRelocation && player.getHeldItem() != null && RefinedRelocationHelper.isSortingUpgrade(player.getHeldItem())){
        val l:TileLocker = tileEntity.asInstanceOf[TileLocker];
  ...
  def canPlaceBlockAt(itemBlock:ItemStack, world:World, x:Int, y:Int, z:Int, reversed:Boolean):Boolean={
      
    val dirs:Array[ForgeDirection] = TileLocker.validDirections;
    var chests:Int = 0;
    for(dir <- dirs){
      val tile:TileEntity = world.getTileEntity(x+dir.offsetX, y+dir.offsetY, z+dir.offsetZ);
      if(tile == null || !tile.isInstanceOf[TileLocker]){}
      else{
        val chest:TileLocker = tile.asInstanceOf[TileLocker];
        if(!chest.isValidForConnection(itemBlock, reversed)){}
