 ...
 * @author Jacob
 */
class ItemBlockImprovedChest(block:Block) extends ModItemBlockMulti(block){
  ...
  override def onItemUse(stack:ItemStack, player:EntityPlayer, world:World, x:Int, y:Int, z:Int, side:Int, hitX:Float, hitY:Float, hitZ:Float):Boolean={
    ...
    else if(!field_150939_a.asInstanceOf[BlockImprovedChest].canPlaceBlockAt(stack, world, x1, y1, z1))
    {
      return false;
    }
