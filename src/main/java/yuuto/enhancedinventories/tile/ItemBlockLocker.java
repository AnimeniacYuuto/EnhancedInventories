...
public class ItemBlockLocker extends ModItemBlockMulti{
	...
	public boolean onItemUse(ItemStack stack, EntityPlayer player, World world, int x, int y, int z, int p_77648_7_, float p_77648_8_, float p_77648_9_, float p_77648_10_)
    {
        ...
        else if (y == 255 && this.field_150939_a.getMaterial().isSolid())
        {
        	...
        	double angle = BlockLocker.getRotation(dir.getOpposite());
    		double yaw = ((player.rotationYaw % 360) + 360) % 360;
    		
	        if(!((BlockLocker)field_150939_a).canPlaceBlockAt(stack, world, x, y, z, BlockLocker.angleDifference(angle, yaw) > 0))
	        {
	        	return false;
	        }
